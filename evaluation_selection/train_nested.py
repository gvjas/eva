...
def train(dataset_path: Path, save_model_path: Path, test_size: float, random_state: int,
    ...
    if model == 'rfc':
        ...
        for i in range(knested):
            with mlflow.start_run():
                mlflow.log_param("model", model.upper() + '_nested_cv')
                mlflow.log_param("n_neighbors", scores['best_params'][i].get('n_neighbors', n_neighbors))
                mlflow.log_param("weights", scores['best_params'][i].get('weights', weights))
                mlflow.log_param("algorithm", scores['best_params'][i].get('algorithm', algorithm))
                mlflow.log_param("use_threshold", use_threshold)
                mlflow.log_param("use_scaler", use_scaler)
                mlflow.log_param("n_pca", n_pca)
                mlflow.log_param("use_sfm", use_sfm)
    ...
    with mlflow.start_run():
        ...
        click.echo(f"Accuracy: {acc} ({acc_std}),\n"
